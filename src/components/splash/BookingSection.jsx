...
export default function BookingSection({ preSelectedExperience }) {
  ...
  const timeSlots = settings?.time_slots?.length
    ? settings.time_slots
  ...
  useEffect(() => {
    ...
    const keyword = preSelectedExperience.toLowerCase();
  ...
  const subExps = selectedExpObj?.price_table || [];

  // Birthday pack only available for these experiences
  const BIRTHDAY_ALLOWED_SLUGS = ['splash', 'phone-case', 'group-splash', 'pour'];
  ...
  const isWhatsAppOnly = (selectedExpObj?.whatsapp_only || false) && !(birthdayPack && !birthdayWhatsAppOnly);
  ...
  useEffect(() => {
    ...
    db.getSlotAvailability(selectedExpObj.slug, form.date, form.subExperience)
      .then(result => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { bookedPerSlot, maxCapacity } = await db.getSlotAvailability(selectedExpObj.slug, form.date, form.subExperience);
      ...
      <div className="max-w-2xl mx-auto relative z-10">
        ...
        {submitted ? (
          ...
          <motion.form initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                ...
                <Select value={form.subExperience} onValueChange={handleSubChange}>
                  ...
                  <SelectContent className="bg-obsidian border-white/10">
                    {subExps.map((s, idx) => {
                      ...
                      return (
                        <SelectItem key={name || idx} value={name} className="text-white focus:bg-white/10 focus:text-white">
                          {name}
                        </SelectItem>
                      );
                    })}
                ...
                <SelectContent className="bg-obsidian border-white/10">
                  ...
                  {birthdayPack && (
                    <SelectItem value="20+" className="text-neon-pink focus:bg-white/10 focus:text-white">
                      20+ {isAr ? '(واتساب فقط)' : '(WhatsApp only)'}
                    </SelectItem>
                  )}
                </SelectContent>
              </Select>
            ...
            {/* Birthday Pack Toggle */}
            {showBirthdayPack && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                className={`rounded-xl p-4 flex items-center justify-between gap-4 cursor-pointer transition-all ${
                  birthdayPack ? 'bg-neon-pink/20 border border-neon-pink/50' : 'bg-white/5 border border-white/10'
                }`}
                onClick={() => setBirthdayPack(!birthdayPack)}>
                <div className="flex items-center gap-3">
                  <Gift className={`w-6 h-6 ${birthdayPack ? 'text-neon-pink' : 'text-white/50'}`} />
                  <div>
                    <p className={`font-heading font-bold text-sm ${birthdayPack ? 'text-neon-pink' : 'text-white/70'}`}>
                      {isAr ? 'إضافة باقة عيد الميلاد 🎂' : 'Add Birthday Pack 🎂'}
                    </p>
                    <p className="text-white/40 text-xs font-body">
                      {isAr ? 'رسوم الباقة 60 ريال للمجموعة الكاملة (وليس للشخص)' : 'Pack fee 60 SAR for the whole group (not per person)'}
                    </p>
                ...
                <div className={`w-12 h-7 rounded-full flex items-center px-1 transition-all ${birthdayPack ? 'bg-neon-pink justify-end' : 'bg-white/10 justify-start'}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2 sm:col-span-2">
                ...
                <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  ...
                  className="bg-white/5 border-white/10 text-white h-12 rounded-xl w-full placeholder:text-white/30" />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}
                  placeholder={isAr ? 'رقم الجوال' : 'Your phone'}
                  className="bg-white/5 border-white/10 text-white h-12 rounded-xl w-full placeholder:text-white/30" />
              </div>
            </div>
            ...
            {/* Error */}
            {bookingError && (
              <div className="flex items-center gap-2 text-red-400 text-sm font-body bg-red-400/10 border border-red-400/20 rounded-xl p-3">
            ...
            {/* Submit */}
            {isWhatsAppOnly || birthdayWhatsAppOnly ? (
              <Button type="button" onClick={() => handleWhatsAppRedirect(birthdayWhatsAppOnly)}
                className="w-full h-14 rounded-xl bg-neon-green text-obsidian font-heading font-bold text-lg hover:bg-neon-green/90 transition-all">
                {isAr ? 'احجز عبر واتساب' : 'Book via WhatsApp'} 💬
              </Button>
            ) : (
              <Button type="submit" disabled={isSubmitting}
                className="w-full h-14 rounded-xl bg-gradient-to-r from-neon-pink to-uv-purple text-white font-heading font-bold text-lg hover:opacity-90 transition-all disabled:opacity-50">
                {isSubmitting ? (isAr ? 'جارٍ الحجز...' : 'Booking...') : tr(lang, 'booking_submit')}
              </Button>
            )}
          </motion.form>
        )}
